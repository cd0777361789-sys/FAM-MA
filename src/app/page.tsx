  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--cream)' }}>
          ...
          <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="grid grid-cols-2 gap-4 max-w-xl mx-auto">
              {categories.map(cat => {
                ...
                const bgs: Record<string, string> = { clothing: 'linear-gradient(135deg, #F9E8D9, #FDF8F0)', jewelry: 'linear-gradient(135deg, #E8D5C4, #FDF8F0)' };
                return (
                  <a key={cat.id} href="#products" onClick={() => setActiveFilter(cat.name_ar)} className="group relative rounded-2xl overflow-hidden bg-white transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                    <div className="aspect-[4/3] flex items-center justify-center" style={{ background: bgs[cat.slug] || bgs.clothing }}>
                      {cat.image ? (
                        <img src={cat.image} alt={cat.name_ar} className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-5xl group-hover:scale-110 transition-transform duration-300">{emojis[cat.slug] || '🛍️'}</span>
                      )}
                    </div>
                    <div className="p-4 text-center">
                      <h3 className="font-bold text-base mb-0.5" style={{ color: '#2C1810' }}>{cat.name_ar}</h3>
                      <p className="text-xs" style={{ color: '#A67B5B' }}>{cat.description_ar}</p>
                    </div>
                  </a>
                );
              })}
      ...
      {settings.site_whatsapp && (
        <a href={`https://wa.me/${settings.site_whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="whatsapp-float" title="تواصلي عبر الواتساب">
