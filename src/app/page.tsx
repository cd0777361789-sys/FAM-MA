    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--cream)' }}>
          ...
          <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {categories.map(cat => {
                ...
                return (
                  <Link key={cat.id} href={`/category/${cat.slug}`} className="group flex items-center gap-2.5 px-5 py-3 rounded-2xl bg-white shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5" style={{ border: '1.5px solid #E8C9A0' }}>
                    <span className="w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0 transition-transform group-hover:scale-110" style={{ background: 'linear-gradient(135deg, #FDF8F0, #F5EDE0)' }}>{emojis[cat.slug] || '🛍️'}</span>
                    <div className="text-right">
                      <h3 className="font-bold text-sm" style={{ color: '#2C1810' }}>{cat.name_ar}</h3>
                      {cat.description_ar && <p className="text-[10px] leading-tight mt-0.5" style={{ color: '#A67B5B' }}>{cat.description_ar}</p>}
                    </div>
                    <svg className="w-4 h-4 mr-1 flex-shrink-0 opacity-30 group-hover:opacity-70 transition-all group-hover:-translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
                  </Link>
                );
              })}
            </div>
      ...
      {settings.site_whatsapp && (
        <a href={`https://wa.me/${settings.site_whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="fixed bottom-6 left-6 z-[999] group" title="تواصلي عبر الواتساب">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg transition-all duration-300 group-hover:scale-105 group-hover:shadow-xl group-hover:-translate-y-1" style={{ background: 'linear-gradient(135deg, #25D366, #128C7E)' }}>
          ...
          <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-[#2C1810] text-white text-[10px] font-bold px-2.5 py-1 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">واتساب</span>
        </a>
      )}
    </div>
  );
